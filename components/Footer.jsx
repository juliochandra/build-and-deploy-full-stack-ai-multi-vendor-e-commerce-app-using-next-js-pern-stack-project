  ...
  return (
    <footer className="mx-6 bg-white">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col items-start justify-between gap-10 border-slate-500/30 border-b py-10 text-slate-500 md:flex-row">
          <div>
            <Link href="/" className="font-semibold text-4xl text-slate-700">
            ...
            <div className="mt-5 flex items-center gap-3">
              {socialIcons.map((item, i) => (
                <Link
                  href={item.link}
                  key={i}
                  className="flex h-10 w-10 items-center justify-center rounded-full border-slate-300 bg-slate-100 transition hover:scale-105 hover:border"
                >
                  <item.icon />
                </Link>
              ))}
          ...
          <div className="flex w-full flex-wrap justify-between gap-5 text-sm md:w-[45%]">
            {linkSections.map((section, index) => (
              <div key={index}>
                <h3 className="mb-3 font-medium text-slate-700 md:mb-5">
                  {section.title}
                </h3>
                <ul className="space-y-2.5">
                  {section.links.map((link, i) => (
                    <li key={i} className="flex items-center gap-2">
                      {link.icon && <link.icon />}
                      <Link href={link.path} className="transition hover:underline">
                        {link.text}
                      </Link>
                    </li>
                  ))}
